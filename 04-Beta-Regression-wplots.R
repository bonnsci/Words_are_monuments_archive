#Beta--regression analysis and plots
...
df <- df %>%
  mutate_at(vars(problem),  
            ~recode(.,  "Named for person who directly or used power to perpetrate violence against a group" = "Anti-Indigenous genocide perpetrator", 
...
df.map.long <- read.csv("./Data/Generated/df_map_long2020-11-16.csv", header = TRUE)
...
levels(df.map.long$problem)
# Anti-Indigenous genocide perpetrator (AIGP)
AIGP <- subset(df.map.long, problem=="Named for person who directly or used power to perpetrate violence against a group")  #anti-indigenous genocide perpetrator
FRP <- subset(df.map.long, problem=="Named after person who supported racist ideas (but non-violent, not in power)") # for a racist person
MC <- subset(df.map.long, problem=="Colonialism - non-violent person but gained from Indigenous removal") #memorializes colonization
PR <- subset(df.map.long, problem=="Name itself promotes racist ideas and/or violence against a group")  #promotes racism 
WU <- subset(df.map.long, problem=="Western use of Indigenous name")  

#' 
#' ## Analysis of "Anti-Indigenous genocide perpetrator"
#' 
#' We are testing for the effect of longitude on Anti-Indigenous genocide perpetrator (AIGP) using a beta regression model.
#' 
## ----message=F, warning=F------------------------------------------------
...
#### PLOT FITTED VALUES AND RESIDUALS BY CATEGORY.
### BREAK UP BY DIFFERENT PROPORTIONS/CATEGORIES
### IDENTIFY A SPECIFIC 
### PERCENT~LONGITUDE+YEAR (PROBABLY CORRELATED THOUGH)
### HAVE YEAR IN THE BACK POCKET...
...
addThese <- rename(addThese, c(fit = "y", se.fit = "SE"))
...
ggplot(data=MC, aes(x=X, y=percent_percent)) +
  ...
  geom_line(data = addThese, aes(x=X, y=lwr), color = "red", linetype = "dashed") + 
  geom_line(data = addThese, aes(x=X, y=upr), color = "red", linetype = "dashed") + 
  theme(
    panel.grid.minor=element_blank(), 
    panel.grid.major=element_blank() ,
    panel.background = element_rect(fill = 'gray90') ,
...
MC.plot.year <- ggplot(data=MC, aes(x=yr, y=percent_percent)) +
  geom_point(show.legend=T) +
  xlab("Year") +
  ylab("Proportion of Park Names") +
  # facet_grid(rows=vars(problem), scales="free_y") +
...
#' 
#' **Conclusion on WU.**
#' Based on the log-likelihood ratio test, we rejected the null hypothesis and and conclude that the alternative hypothesis is true at the 95% confidence level. Specifically, we found that longitude is a significant predictor of the proportion of place names associated with the category "Western Use" (WU) across the US national parks studies (*X^2^*(1) = 3.8414, *p* = 0.05). Specifically, for each  1 degree increase in longitude the proportion of place names associated with western use decreases by 0.02 (95% CI = -0.03, -0.005).
#' 
## ------------------------------------------------------------------------
# use gam and mgcv functions to generate plots
...
WU.plot.year <- ggplot(data=WU, aes(x=yr, y=percent_percent)) +
  geom_point(show.legend=T) +
  xlab("Year") +
  ylab("Proportion of Park Names") +
  xlim(min(WU$yr - 1), max(WU$yr + 1)) +
  ylim(0, max(WU$percent_percent+0.02)) +
  theme(
    panel.grid.minor=element_blank(), 
    panel.grid.major=element_blank() ,
...
# Model for IPN
model_v2 = betareg(percent_percent ~ X+i_or_w+X:i_or_w,
                   data = df.map.long2, link = "log")

joint_tests(model_v2)
...
test <- ggarrange(MC.plot, MC.plot.year,
                  WU.plot, WU.plot.year, 
                  labels = c("A", "B",
                             "C", "D"),
                  ncol = 2, nrow = 2,
                  heights = c(2,2))

ggsave("./outputs/figs/scatter-betareg_4facets.png", test, width = 10, height = 10)
...
MC.plot_forpub <- MC.plot + 
  ylab("Proportion of park names") +
  xlab("Longitude") +
  geom_label_repel(data=labs_MC, aes(label=short),
                   segment.color="#40B0A6",   # "steelblue1"
                   #segment.size = 0.5,
                   box.padding=1,
                   #fill=alpha("gray100",0.8),
                   color="#40B0A6",
                   size=3,
                   #nudge_x = -5,
                   nudge_y = 0.04) + 
  annotate("text", x=-140, y=0.15, label="Memorialize\ncolonialism", size=3.5, fontface="bold") +
  theme(plot.margin = unit(c(0.5,0.5,0.0,0.5), "cm"),
        legend.position="none",
        axis.title.y=element_text(size=10, color="black", face="bold"),
        axis.text.y=element_text(size=9, color="black", margin=margin(l=0, unit="pt")),
...
test_longonly <- ggarrange(MC.plot_forpub, 
                           WU.plot_forpub,  
                           labels = c("A", 
                                      "B"),
...
quartz(width=3, height=7) # might still need to adjust window proportions before saving
test_longonly
quartz.save("./outputs/figs/scatter-betareg_2longfacets.png", type="png", device=dev.cur(), dpi=300, bg="white")
dev.off()
